...
func TestNoNewKargoAgentDataSourceFields(t *testing.T) {
	...
	assert.Equal(t, reflect.TypeOf(types.KargoAutoscalerConfig{}).NumField(), len(getKargoAutoscalerConfigDataSourceAttributes()))
	assert.Equal(t, reflect.TypeOf(types.KargoControllerAutoScalingConfig{}).NumField(), len(getKargoControllerAutoScalingConfigDataSourceAttributes()))
	assert.Equal(t, reflect.TypeOf(types.KargoResources{}).NumField(), len(getKargoResourcesDataSourceAttributes()))
}
