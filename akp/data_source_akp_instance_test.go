		...
		Steps: []resource.TestStep{
			// Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttrWith("data.akp_instance.test", "argocd_resources.0", func(value string) error {
						if !strings.Contains(value, "argocd-example-apps.git") {
							return fmt.Errorf("expected to contain repoURL")
						}
						return nil
					}),
					resource.TestCheckResourceAttrWith("data.akp_instance.test", "argocd_resources.1", func(value string) error {
