...
The resource's `spec.data.kustomization` stores user-supplied configuration. When
`custom_agent_size_config` is set and `kustomization` is omitted, the generated
sizing patches are not stored in this attribute, which can be null. The
`akp_cluster` data source exposes the API's generated kustomization.

Do not duplicate the resource patches generated by `custom_agent_size_config`
in `kustomization`. Use `kustomization` for unrelated user customizations.

For existing state and configuration migration, see the
[version 0.17 upgrade guide](../guides/v0.17-upgrading.md).
