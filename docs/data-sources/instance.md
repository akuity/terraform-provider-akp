...
- `allowed_clusters` (List of String) Names of managed clusters the secret is synced to.
- `cluster_selector` (String) Kubernetes label selector that selects the managed clusters the secret is synced to.
- `labels` (Map of String) Additional labels set on the secret.
- `secret_keys` (List of String) Names of keys stored in the secret. Secret values are not returned.
